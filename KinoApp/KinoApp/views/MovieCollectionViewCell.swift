...
final class MovieCollectionViewCell: UICollectionViewCell {
    ...
    private lazy var genresLabel: UILabel = {
        let label = UILabel()
        let genres = ["Comedy", "Thriller", "History", "Action", "Romance", "Drama"]
        let randomGenres = genres.shuffled().prefix(3)
        label.text = randomGenres.joined(separator: ", ")
    ...
    func configure(with movie: MovieModel) {
        guard let url = URL(string: "\(itemForProject.Links.imageUrl)\(movie.posterPath)") else {fatalError("Incorrect link to poster path!")}
